...
import dao.WhiskiesDAO.{ Whiskies }
...
object ReviewsDAO {
  ...
  class Reviews(_tableTag: Tag) extends profile.api.Table[ReviewsRow](_tableTag, "reviews") {
    ...
    def ? = (reviewId, Rep.Some(whiskyId), Rep.Some(score), comment, postedDate).shaped.<>({r=>import r._; _1.map(_=> ReviewsRow.tupled((_1, _2.get, _3.get, _4, _5)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
  ...
  /* 素のSQL文発行時のための変換定義 */
  implicit def GetResultReviewsRow(implicit e0: GR[Option[Int]], e1: GR[Int], e2: GR[Short], e3: GR[Option[String]], e4: GR[Option[LocalDate]]): GR[ReviewsRow] = GR{
    prs => import prs._
